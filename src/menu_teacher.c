/* menu_teacher.c - 教师控制台菜单，含课程管理、选课名单、成绩录入 */
        ...
        if(ch==1) {
            ...
            fgets(course_id,sizeof(course_id),stdin);course_id[strcspn(course_id,"\n")]='\0';str_trim(course_id);
            {
                ...
                if(!ds_course_find_by_id(course_id,&c)){printf("\n  [提示] 课程不存在\n");}
                    ...
                    sprintf(path,"data/exports/%s_名单.txt",course_id);
#ifdef _WIN32
                    _mkdir("data/exports");
#else
...
static void sub_grade_mgr(struct Session* s)
{
    ...
    while(1) {
        ...
        if(ch==1) {
            ...
            ds_course_load_all(my_courses, &total);
            printf("\n  我的课程:\n"); my_count=0;
            for(i=0;i<total;i++){if(strcmp(my_courses[i].teacher_id,s->userid)==0){my_count++;printf("    %d. %s - %s (已选%d人)\n",my_count,my_courses[i].id,my_courses[i].name,my_courses[i].enrolled);}}
            if(my_count==0){printf("\n  暂无课程。\n");pause_and_continue();}
            else{
                int cno=get_choice("  请选择课程编号(0取消): ",0,my_count);
                if(cno==0){}
                    ...
                    for(i=0;i<total;i++){if(strcmp(my_courses[i].teacher_id,s->userid)==0){my_count++;if(my_count==cno){picked=&my_courses[i];break;}}}
                    if(picked==NULL){printf("\n  [错误]\n");pause_and_continue();}
                        ...
                        for(si=0;si<sc_total;si++){
                            if(strcmp(all_scores[si].course_id,picked->id)==0){
                                struct Student st;
                                if(!shown){printf("  %-12s %-8s %-6s %-6s %-6s %s\n","学号","姓名","平时","期末","总评","绩点");printf("  -----------------------------------------------------\n");shown=1;}
                                ds_student_find_by_id(all_scores[si].student_id,&st);
                                printf("  %-12s %-8s %-6.1f %-6.1f %-6.1f %.2f\n",all_scores[si].student_id,st.name,all_scores[si].daily_score,all_scores[si].final_score,all_scores[si].total_score,all_scores[si].gpa);
                                score_count++;
                            }
                        }}
                        if(score_count==0)printf("  暂无成绩数据。\n");
                        else{
                                ...
                                if(sno==0){}
                                    ...
                                    if(is_enter&&roster[sno].has_grade){printf("\n  [提示] 该学生已有成绩(总评:%.1f)\n",roster[sno].total);pause_and_continue();}
                                        ...
                                        if(is_enter){
                                            ...
                                            if(ds_score_add(&sc))printf("\n  [成功] %s 成绩已录入: 总评%.1f 绩点%.2f\n",roster[sno].sname,sc.total_score,sc.gpa);else printf("\n  [失败]\n");
                                        } else {
